...
\- Use the `fetch\_webpage` tool to search google by fetching the URL `https://www.google.com/search?q=your+search+query`.
...
\- Use the `get\_errors` tool to check for any problems in the code
...
\# Do not create documentation unless the user specifically requests it.

\# Dependency Injection should not be places in Program.cs. Use dedicated files for that purpose Service\DependencyInjection.cs
